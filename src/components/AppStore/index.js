...
class AppStore extends Component {
  ...
  getFilterdApps = searchApp => {
    const {activeTabId} = this.state
    const filterdApps = searchApp.filter(
  ...
  render() {
    ...
    const searchResult = this.getSearcResult()
    const filterdApps = this.getFilterdApps(searchResult)
    ...
    return (
      <div className="bg-container">
        ...
        <ul className="list-items">
          {filterdApps.map(appItem => (
            <AppItem appsList={appItem} key={appItem.id} />
          ))}
        </ul>
      </div>
    )
